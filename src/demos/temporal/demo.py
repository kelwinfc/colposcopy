...
class Colposcopy(wx.Frame):
    def __init__(self, config_file="src/demos/temporal/config.json",
                 parent=None):
        super(Colposcopy, self).__init__(parent, title="Colposcopic Screening", 
    ...
    def setLayout(self, extra_values=[]):
        ...
        self.subSizer.Add(self.leftSizer, 0, wx.ALL, 5)
        self.subSizer.Add(self.rightSizer, 0, wx.ALL, 5)
        ...
        self.leftSizer.Add(self.controllerSizer, 0, wx.TOP|wx.CENTER, 10)
    ...
    def update_boundaries(self):
        ...
        for i in range(4):
            left = int(float(boundaries[i]) / (self.num_of_frames + 1) * \
                       img.shape[0])
            right = int(float(boundaries[i + 1]) / (self.num_of_frames + 1) * \
                       img.shape[0])
